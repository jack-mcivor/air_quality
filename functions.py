...
def find_fullest_csv(csv_links: list, year: int):
...
def get_air_quality_from_eea(city_name: str, pollutant: str, start_year: str, end_year: str):
    """
    ...
    for year in range(int(start_year), int(end_year) + 1):
        try:
            ...
            print(f"!! Missing data for {year} for {city_name} city.")
            pass
    ...
    res_df = res_df.rename(columns={"DatetimeBegin": "date", "Concentration": pollutant})
...
##################################### USEPA
city_code_dict = {}
pollutant_dict = {"CO": "42101", "SO2": "42401", "NO2": "42602", "O3": "44201", "PM10": "81102", "PM2.5": "88101"}
...
def get_city_code(city_name: str):
    ...
    if city_code_dict:
        ...
        params = {"email": "test@aqs.api", "key": "test"}
...
def get_air_quality_from_usepa(city_name: str, pollutant: str, start_date: str, end_date: str):
    """
    Takes city name, daterange and returns pandas DataFrame with daily air quality data.
    ...
    for start_date_, end_date_ in make_date_intervals(start_date, end_date):
        ...
        if pollutant == "pm2.5":
            pollutant = "pm2_5"
        df_ = df_.rename(columns={"date_local": "date", "arithmetic_mean": pollutant})
...
def get_weather_data_from_open_meteo(
    city_name: str, start_date: str, end_date: str, coordinates: tuple[float, float] | None = None, forecast: bool = False
):
    """
...
def get_aqi_data_from_open_meteo(
    city_name: str, start_date: str, end_date: str, coordinates: tuple[float, float] | None = None, pollutant: str = "pm2_5"
):
    """
    ...
    # rename columns
    res_df = res_df.rename(columns={"time": "date"})

    res_df["city_name"] = city_name

    # change columns order
    res_df = res_df[["city_name", "date", pollutant]]
    end_of_cell = time.time()
    print(f"Processed {pollutant.upper()} for {city_name} since {start_date} till {end_date}.")
    print(f"Took {round(end_of_cell - start_of_cell, 2)} sec.\n")
