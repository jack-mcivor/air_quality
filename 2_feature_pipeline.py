    ...
    for continent in target_cities:
        for city_name, coords in target_cities[continent].items():
            df_ = get_aqi_data_from_open_meteo(
                city_name=city_name, coordinates=coords, start_date=str(hindcast_day), end_date=str(today)
            )
...
def g():
    ...
    air_quality_fg = fs.get_or_create_feature_group(name="air_quality", version=1)
    weather_fg = fs.get_or_create_feature_group(name="weather", version=1)
